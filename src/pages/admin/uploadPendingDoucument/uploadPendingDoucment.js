...
import {
  selectTransactionsRaisedByMeDataMap,
  selectTransactionsSentToMeDataMap,
} from "store/UploadPendingDocList/UploadPendingDocList.selecter"
...
import { SrNo } from ".././company-search/companyssearchColl"
...
const UploadPendingListModule = props => {
  ...
  const selectTransactionsRaisedByMe = useSelector(
    selectTransactionsRaisedByMeDataMap
  )
  const selectTransactionsSentToMe = useSelector(
    selectTransactionsSentToMeDataMap
  )
  ...
  const DueSincedate = cell => {
    ...
    const newDate =
      cell.cell.row.original.paymentDate != undefined
        ? cell.cell.row.original.paymentDate.split("-").reverse().join("-")
        : ""
    ...
    return (
      <div className="" style={{ padding: "2px 10px", fontSize: "12px" }}>
        ...
        >
          ...
          <div className="text-capitalize">
            {cell.cell.row.original.paymentDate}
          </div>
        </div>
      </div>
    )
        ...
        Cell: cellProps => {
          return (
            <div className="d-flex text-capitalize">
              {cellProps.cell.row.original.creditor.companyName}
              <br />
              {cellProps.cell.row.original.creditor.address1}{" "}
              {cellProps.cell.row.original.creditor.address2},{" "}
              {cellProps.cell.row.original.creditor.city}
            </div>
          )
        },
      },
            ...
            <div className="d-flex">
              <div className="pt-2">
                ...
                <button
                  ...
                  target="_blank"
                  onClick={() => {
                    handleUploadFiles(cellProps.cell.row.original),
        ...
        Cell: cellProps => {
          return (
            <div className=" d-flex text-capitalize">
              ...
              {cellProps.cell.row.original.debtor.address2 == undefined
                ? ""
                : cellProps.cell.row.original.debtor.address2 + ", "}{" "}
              {cellProps.cell.row.original.debtor.city},{" "}
              {cellProps.cell.row.original.debtor.state}
            </div>
          )
        },
      },
  ...
  return (
    <React.Fragment>
      <div className="page-content">
        <Container fluid>
          <Breadcrumbs title="Upload Pending File" breadcrumbItem="Upload Pending File" />
          {uploadFilesModalShow && (
        <UploadPendingDocModel
          isOpen={uploadFilesModalShow}
          toggle={toggleUploiadFiles}
          uploadFilesModelDataForUpload={uploadFilesModelDataForUpload}
