...
export const ConfirmDefaulterModule = props => {
  ...
  const { allInvoiceLists, handleCheckboxChange, isChecked, feedbackPayload, moduleName, defaulterId, handlePrevious, handleNext, toggle } = props
    ...
    <div className="mt-5">
      ...
      <ModalFooter className="mt-5">
        <Row className="backBtn ">
          <Col  ><Button className="btn btn-info" onClick={() => handlePrevious()}><span className="h5">Back</span></Button></Col>
          <Col style={{ display: 'flex', gap: '1rem', justifyContent: 'flex-end' }}>
            <Button className="text-center btn btn-secondary " onClick={() => handleDraftInvoice()}><span className="h5">Save as Draft</span></Button>
            <Button className="text-center btn btn-info" onClick={() => handleSubmit()} disabled={!isChecked}><span className="h5">Submit</span></Button>
          </Col>
        </Row>
      </ModalFooter>
