...
export const InvoiceDetailsModule = ({
  filteredCustomerDetail,
  selectedOption,
  isChangedCustomername,
  handleNext,
  data,
  setData,
  newPayload,
  ...
  const CreatePayload = () => {
    const newData = data.map((selectedData, index) => {
      ...
      return {
        ...
        billNumber: "",
        creditAmount:
          selectedData.amount !== undefined ? Number(selectedData.amount) : "",
        remainingAmount:
          selectedData.amount !== undefined ? Number(selectedData.amount) : "",
        status: "OPEN",
        interestRate: "",
        ...
        referenceNumber: "",
        invoiceNumber:
          selectedData.itemDetail !== undefined ? selectedData.itemDetail : "",
        dueDate: selectedData.date ? convertedDate : date.format("YYYY-MM-DD"),
        percentage: "",
        purchaseOrderDocument: selectedData.purchaseOrderDocument
          ? selectedData.purchaseOrderDocument.documentId
          : "",
        challanDocument: selectedData.DispatchDocument
          ? selectedData.DispatchDocument.documentId
          : "",
        invoiceDocument: selectedData.invoiceDocument
          ? selectedData.invoiceDocument.documentId
          : "",
        transportationDocument: selectedData.DeliveryDocument
          ? selectedData.DeliveryDocument.documentId
          : "",
      }
    })
  ...
  function uploadFile(formData, index, folderName) {
    ...
    axios
      .post(
        "https://msmesuraksha-backend.azurewebsites.net/api/files/upload",
        formData,
        {
          headers: headers,
        }
      ...
      .then(response => {
        ...
        if (
          response.data.response.fieldName == "uploadchallanDispatchDocument"
        ) {
          const newData = [...data]
          const responses = response.data.response
        ...
        if (
          response.data.response.fieldName ==
  ...
  return (
    <div>
      {filteredCustomerDetail.length != 0 && (
          ...
          <div className="mt-3">
                    ...
                    <CardBody>
                      ...
                      <Row className="">
                        <Col md={4} className="p-2">
                          ...
                          {row.invoiceDocument != null &&
                          row.invoiceDocument != "" ? (
                            <>
                              <Row>
                                <Col md={5}>
                                  {row.invoiceDocument.name == undefined
                                    ? row.invoiceDocument.folderName
                                    : row.invoiceDocument.name}
                                </Col>
                                <Col md={6}>
                                  {" "}
                                  <button
                                    ...
                                    onClick={() =>
                                      handleCancel("invoiceDocument", index)
                                    }
                                  >
                                    Delete
                                  </button>
                        ...
                        <Col md={4} className="p-2">
                          <Label>
                            {" "}
                            <strong>Challan / Dispatch Document</strong>
                          </Label>
                          {row.DispatchDocument != null &&
                          row.DispatchDocument != "" ? (
                            <>
                              <Row>
                                <Col md={5}>
                                  {row.DispatchDocument.name == undefined
                                    ? row.DispatchDocument.folderName
                                    : row.DispatchDocument.name}
                                </Col>
                                <Col md={6}>
                                  {" "}
                                  <button
                                    className="btn btn-sm btn-danger"
                                    onClick={() =>
                                      handleCancel("DispatchDocument", index)
                                    }
                                  >
                                    Delete
                                  </button>
                        ...
                        <Col md={4} className="p-2">
                          <Label>
                            {" "}
                            <strong>Delivery / Transportation Document</strong>
                          </Label>
                          {row.DeliveryDocument != null &&
                          row.DeliveryDocument != "" ? (
                            <>
                              <Row>
                                <Col md={5}>
                                  {row.DeliveryDocument.name == undefined
                                    ? row.DeliveryDocument.folderName
                                    : row.DeliveryDocument.name}
                                </Col>
                                <Col md={6}>
                                  {" "}
                                  <button
                                    className="btn btn-sm btn-danger"
                                    onClick={() =>
                                      handleCancel("DeliveryDocument", index)
                                    }
                                  >
                                    Delete
                                  </button>
                      ...
                      <Row>
                        <Col md={4} className="p-2">
                          ...
                          {row.purchaseOrderDocument != null &&
                          row.purchaseOrderDocument != "" ? (
                            <>
                              <Row>
                                <Col md={5}>
                                  {row.purchaseOrderDocument.name == undefined
                                    ? row.purchaseOrderDocument.folderName
                                    : row.purchaseOrderDocument.name}
                                </Col>
                                <Col md={6}>
                                  {" "}
                                  <button
            ...
            <Row>
              <Col md={12} className="text-end">
                <Button className="btn btn-info" onClick={addFaqsRow}>
                  Add Another Invoice
                </Button>
              </Col>
            </Row>
            <Row className="text-end mt-3">
              <Col md={12}>
                <h5>
                  {" "}
                  <div> Total Amount = {numberFormat(total)}</div>
                </h5>
              </Col>
            </Row>
          </div>
