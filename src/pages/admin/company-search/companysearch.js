...
import {
  fetchCompanySearchStart,
  getAllCompanyListAction,
} from "store/CompanySearch/CompanySearch.action"
import {
  selectCompanySearchList,
  selectdashboardAdminDataMap,
  getAllCompanyListSelector,
  selectCompanySearchLoder,
} from "store/CompanySearch/CompanySearch.selecter"
...
const CompanySearch = props => {
    ...
    () => [
      {
        ...
        Cell: cellProps => {
          return (
            <div className="company-name-cell" style={{ cursor: "pointer" }}>
  ...
  const handleFilter = filters => {
    const filtereCompany = selectCompanySearchListMap.filter(item =>
      item.CompanyName.toLocaleLowerCase().includes(
        filters.company.toLocaleLowerCase()
      )
    )

    const filteredgstMatch = filtereCompany.filter(item =>
      item.GST.toLocaleLowerCase().includes(filters.gst.toLocaleLowerCase())
    )

    const filterePANCARD = filteredgstMatch.filter(item =>
      item.PANCARD.toLocaleLowerCase().includes(filters.pan.toLocaleLowerCase())
    )
  ...
  return (
    <React.Fragment>
      <div className="page-content">
        <Container fluid>
          <Breadcrumbs title="Defaulter search" breadcrumbItem="Defaulter search" />
          {CompanySearchLoder == false ? (
            <Spinner />
          ) : (
            <>
              {modal1 && (
                <CompnayViewDetails
                  ...
                  selected={selected}
                  currenViewList={currentUserViewDetails}
                  selectCompanySearchListMap={selectCompanySearchLists}
                />
              )}
