...
import { Col, InputGroup, Row, Container, Input } from "reactstrap"
...
const DiputedBillings = props => {
  ...
  const getDays = () => {
    GetAllInvoice != undefined
      ? GetAllInvoice.map(item => {
          const a = moment(
            item.debtor != null ? item.debtor.createdAt : ""
          ).format("YYYY-MM-DD")
  ...
  function getDebtrosLists(responsData) {
    return (
      ...
      responsData.map(item => {
        return {
          value: item.id,
          label: item.firstname + ", " + item.companyName,
        }
      })
    )
  }
  ...
  const DueSincedate = cell => {
    ...
    const currentDate = new Date(
      cell.cell.row.original.debtor != null
        ? cell.cell.row.original.debtor.createdAt
        : ""
    )
    let e = ""
    ...
    return (
      ...
      >
        ...
        >
          ...
          <div className="text-capitalize">
            {moment(cell.cell.row.original.invoices[0].dueDate).format(
              "DD-MM-YYYY"
            )}
          </div>
        </div>
  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        ...
        Cell: cellProps => {
          const address =
            cellProps.cell.row.original.Address.substring(0, 20) + "..."
          return (
  ...
  return (
    <React.Fragment>
      <div className="page-content">
        <Container fluid>
          <Breadcrumbs title="Record Payment" breadcrumbItem="Record Payment" />{" "}
