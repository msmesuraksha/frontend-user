...
const ApprovedTransaction = props => {
  ...
  const handleFilterdata = (filters) => {
    if (selectCompaintsByMeList) {
      if (filters === "") {
        setFilteredData(selectCompaintsByMeList)
      } else {
        const filteredResults = selectCompaintsByMeList.filter(item => {
  ...
  const getDays = () => {
    selectCompaintsByMeList != undefined ? selectCompaintsByMeList.map((item) => {
